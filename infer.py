...
class Net(pytorch_lightning.LightningModule):
    ...
    def prepare_data(self, data_dir):
        """
        function sets up the correct data path
...
def segment_PETCT(ckpt_path, data_dir, export_dir):
    """
    function performs segmentations
    :param ckpt_path model checkpoint path
    :param data_dir data path
    :param export_dir output path
    """
    print("starting")
...
def run_inference(ckpt_path='/opt/algorithm/epoch=777-step=64573.ckpt', data_dir='/opt/algorithm/',
                  export_dir='/output/'):
    """
    function runs prediction
    :param ckpt_path model checkpoint path
    :param data_dir data path
    :param export_dir output path
    """
    ...
    p_project = '/home/edoli/PycharmProjects/autoPET/'

    classifier = load_model(s_model, p_project)

    segment_PETCT(ckpt_path, data_dir, export_dir)

    # image_test = SimpleITK.GetArrayFromImage(SimpleITK.ReadImage(inputImageFileName))  # mha format for test


def load_model(s_model, p_project):
    """
    function loads model
    :param p_project project path
    :param s_model model name
    """
    print(f'Model {s_model} has been loaded.')
    p_load = p_project + '/contrastive.pkl'
